...
contains a GitLab CI infrastructure developed for RADIUSS projects. RADIUSS
Shared CI is a sub-project of the LLNL RADIUSS project that focuses on
...
RADIUSS projects target the same machines and use the
`Spack packaging system <https://spack.io>`_ to ensure that they
build successfully with similar toolchains.

We designed an automated *CI infrastructure based on GitLab* that is intended
to be sufficiently flexible to be shared among RADIUSS projects. The 
infrastructure involves *using Spack to setup the project dependencies and 
generate build configuration files*. This allows projects to easily 
*share the full context of their builds*. Projects are built and tested 
as usual while most of *the CI infrastructure is shared* to avoid duplication 
...
1. **Use Spack to install dependencies and configure a project build.**
   ...
   setup*, and *build sequence*. Spack is increasingly used to install
   dependency stacks of large simulation codes.
2. **Build and test as you normally do, for the most part.**
